...
void setup() {
  multi = loadImage("multi.jpeg");
  multi.resize(640, 480);
  picNum = 1;
...
void reversemulti() {
  pushMatrix();
  scale(-1, 1);
  curr = multi(cam, multi, threshold, max, mid, low);
  image(curr, -curr.width/2, curr.height/2 );
  popMatrix();
}
...
PImage multi(PImage Image, PImage replacer, int threshold, color max, color mid, color low) {
  PImage multied = new PImage(Image.width, Image.height);
  for (int y=0; y<Image.height; y++) {
    for (int x=0; x<Image.width; x++) {
      for (int i=0; i<3; i++) {
        ...
        if ((distMax < threshold) || (distMid < threshold) || (distLow < threshold)) {
          multied.set(x, y, replacer.get(x, y));
        } else {
          multied.set(x, y, Image.get(x, y));
        }
      }
    }
  }
